function u = optimization_tank(u_ini,state_ini_values,dt,Ref,Np,t)
%In this function we:
...
obj_func = @(u)objfun_tank(u,state_ini_values,dt,Ref,Np,t);
...
%the same function
cons_func = @(u)confun_tank(u,state_ini_values,dt,Ref,Np,t);
%use the fmincon solver
[u,fval,exitflag,output,solutions] = fmincon(obj_func,u_ini,[],[],[],[],[],[],cons_func,ops);


    function J = objfun_tank(u,state_ini_values,dt,Ref,Np,t)
if ~isequal(u,uLast) %check if computation is necessary
% disp('button pressed: objective call');
% pause;
[myJ myG myHeq] = compute_both(u,state_ini_values,dt,Ref,Np,t);
uLast = u;
end
%now compute objective function
J = myJ;
end
    function [G Heq] = confun_tank(u,state_ini_values,dt,Ref,Np,t)
if ~isequal(u,uLast) %check if computation is necessary
